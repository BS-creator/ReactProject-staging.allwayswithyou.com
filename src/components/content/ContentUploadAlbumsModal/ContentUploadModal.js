...
import { FacebookUploadModal, GoogleUploadModal } from '../ContentUploadAlbumsModal';
...
class ContentUploadModal extends Component {
  ...
  render() {
    ...
    return (
        <Modal size='tiny' dimmer='inverted'  open={this.state.openUploadModal}  trigger={<Button id='allContentPage_btnUpload' disabled={isSelectedContent} style={buttonCss.primary} onClick={this.handleUploadModal} > <Icon inverted name='plus'/> {intl.get("uploadAlbums")}</Button>} onClose={() => this.setState({openUploadModal : false})}>
          <Modal.Header>{intl.get("uploadAlbums")}</Modal.Header>
          <Modal.Content image>
